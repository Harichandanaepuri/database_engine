...
public class Table
{
			...
			for(int page = 1; page <= numberOfPages; page ++)
			{
				...
				else
				{
					...
					for(int j = 0; j < cells.length; j++)
					{
						...
						{
							SetPages.setCellOffset(file, page, i, cells[j]);
							i++;
						}
					}
					SetPages.setCellNumber(file, page, (byte)i);
				}
			}

			file = new RandomAccessFile("data\\catalog\\davisbase_columns.tbl", "rw");
			numberOfPages = pag(file);
			for(int page = 1; page <= numberOfPages; page ++)
			{
				file.seek((page-1)*pageSize);
				byte type = file.readByte();
	...
	public static int pag(RandomAccessFile file)
	{
		int num_pages = 0;
		try
		{
	...
	public static void initializeDataStore() 
	{
		...
		try 
		{
			...
			//System.out.println("The first old fileiis"+oldTableFiles[0]);
